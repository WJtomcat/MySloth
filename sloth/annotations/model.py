...
class RootModelItem(ModelItem):
    ...
    def _load(self, index):
        ...
        print(self._children[index])
        fi = ImageFileModelItem(self._children[index])
    ...
    def appendChild(self, item, signalModel=True):
        if isinstance(item, ImageFileModelItem):
            ModelItem.appendChild(self, item, signalModel=signalModel)
        else:
            raise TypeError("Only ImageFileModelItem can be attached to RootModelItem")

    def appendFileItem(self, fileinfo):
        item = ImageFileModelItem(fileinfo)
        self.appendChild(item)
        return item

    def appendFileItems(self, fileinfos):
        items = [ImageFileModelItem(fi) for fi in fileinfos]
...
class KeyValueModelItem(ModelItem, MutableMapping):
    def __init__(self, hidden=None, properties=None):
        ...
        self._hidden.update({None, 'class', 'unlabel', 'unconfirmed'})
        self._seen = False
...
class ImageFileModelItem(KeyValueModelItem):
    ...
    def data(self, role=Qt.DisplayRole, column=0):
        ...
        if role == Qt.DisplayRole:
            if column == 0:
                return('* ' if not self._seen else '') + 'picId:' + str(self['picId']) + ' picName:' + str(self['picName'])
