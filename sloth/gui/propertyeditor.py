...
#
#         self._classbox = QGroupBox('Labels')
...
class PropertyEditor(QWidget):
    ...
    def __init__(self, config, tool, parent=None):
        QWidget.__init__(self, parent)
        self.tool = tool
        self._class_config       = {}
        self._class_items        = {}
    ...
    def onClassButtonPressed(self, label_class):
        if self._class_buttons[label_class].isChecked():
            if self.tool.currentImage() is None:
                self._class_buttons[label_class].setChecked(False)
                return
            self.startInsertionMode(label_class)
        else:
            self.endInsertionMode()
